...
module.exports = function (grunt) {
  ...
  var baseName='appeal-asst-phaseone';
  var localConfig;
  try {
  ...
  grunt.initConfig({
    ...
    auto_install: {
      subdir: {
        options: {
          ...
          failOnError: true,
          npm: '--production',
          bower: false // TODO: Remove this line once TC can actually run
        }
      }
    },
    ...
    compress: {
      dist: {
        options: {
          mode: 'zip',
          archive: 'build/libs/' + baseName + '.zip'
        },
        files: [
          {
            src: [ 'client/**/*.*', 'server/**/*.*', 'node_modules/**/*.*', 'package.json' ],
            dest: '/',
            cwd: 'dist',
            expand: true
          }
      ...
      css: {
        ...
        files: {
          '<%= yeoman.client %>/index.html': [
            ...
            '!<%= yeoman.client %>/{app,components}/**/jquery.dataTables.css'
  ...
  grunt.registerTask('release', [ 'build', 'auto_install', 'compress']);
